...
- **Foundation (Epic 1)** : Supabase Anonymous Auth + SecureStorage + stratégie refresh JWT proactif au foreground.
...
- Supabase Anonymous Auth + `SecureStorage` `expect/actual` + stratégie refresh JWT proactif au foreground (Action item archi #4)
...
As a Colision developer,
I want cross-cutting concerns (Logger, CrashReporter, Analytics, SecureStorage, NotificationPermissionManager, DispatcherProvider, AppError) modeled as Kotlin interfaces in `core/common/` with `Noop` or basic implementations injected via Koin,
So that every feature story consumes these abstractions cleanly, and real impls (Sentry, PostHog) can be swapped in story 1.9.

**Acceptance Criteria:**

**Given** `core/common/`, **When** I list the files, **Then** I find : `Logger.kt`, `CrashReporter.kt` + `NoopCrashReporter.kt`, `Analytics.kt` + `NoopAnalytics.kt`, `SecureStorage.kt` (expect), `NotificationPermissionManager.kt` (expect), `DispatcherProvider.kt`, `AppError.kt` (sealed class), `AppErrorThrowable.kt`, `ResultHelpers.kt` (`appErrorResult`, `foldAppError`).

**Given** Koin, **When** the app starts, **Then** `core/di/CoreModule.kt` registers `single<Logger>`, `single<CrashReporter> { NoopCrashReporter() }`, `single<Analytics> { NoopAnalytics() }`, `single<DispatcherProvider> { DefaultDispatcherProvider() }`. Platform modules register `SecureStorage` and `NotificationPermissionManager` placeholders.
...
### Story 1.8: Anonymous Auth + SecureStorage + JWT refresh proactif

As a Colision developer,
I want Supabase Anonymous Auth plugged in : at first launch `signInAnonymously()` is called, JWT and refresh token stored encrypted via `SecureStorage`, and the session refreshed proactively on foreground when less than 7 days remain on the refresh token,
So that every request has a valid `auth.uid()` and Sophie never sees a "session expired" screen.

**Acceptance Criteria:**

**Given** Android `SecureStorage` actual, **When** I inspect `androidMain/.../SecureStorage.android.kt`, **Then** the impl uses `EncryptedSharedPreferences` with master key from Android keystore. Mirror impl on iOS uses Keychain Services (`kSecAttrAccessibleAfterFirstUnlock`).

**Given** `core/network/SupabaseClientProvider.kt`, **When** I inspect the file, **Then** the Supabase client is built with `Auth` + `Postgrest` + `Functions` modules and a custom `SessionManager` delegating storage to our `SecureStorage`.
...
**Given** cold start with no stored session, **When** `ensureSession()` runs, **Then** `signInAnonymously()` is called, JWT stored, and a row appears in `auth.users` + `device` (via story 1.6 trigger).

**Given** a stored session with refresh token expiring in 5 days, **When** the app returns to foreground, **Then** `refreshIfNeeded()` triggers a refresh and stores the new JWT silently.

**Given** an expired refresh token, **When** `refreshIfNeeded()` fails, **Then** old session is wiped and a new `signInAnonymously()` is called automatically (member rows in previous projects are not auto-recovered at MVP — V2 concern).

### Story 1.9: Sentry + PostHog integration (real impls)

As a Colision developer,
I want `NoopCrashReporter` swapped for `SentryCrashReporter` and `NoopAnalytics` for `PostHogAnalytics`,
So that crashes and product events are captured from day 1.

**Acceptance Criteria:**

**Given** BuildConfig per flavor, **When** I update `BuildConfig.kt` in `development/` and `production/`, **Then** both contain real `sentryDsn` (Sentry Cloud EU, projects `colision-dev` / `colision-prod`) and `posthogApiKey` (PostHog Cloud EU).

**Given** Android `ColisionApplication.onCreate` and iOS `AppDelegate.application(_:didFinishLaunchingWithOptions:)`, **When** the app starts, **Then** Sentry is initialized with correct DSN + environment, PostHog with EU endpoint (`https://eu.i.posthog.com`) and "Disable IP capture" enabled.

**Given** the Koin swap in `core/di/CoreModule.kt`, **When** the bindings resolve, **Then** `CrashReporter` and `Analytics` now provide the real impls (no longer Noop).

**Given** a deliberate exception thrown on dev flavor, **When** it propagates, **Then** within ~1 minute the event appears in Sentry's `colision-dev` project.

**Given** an `Analytics.track("app_opened")` call, **When** it executes, **Then** the event appears in PostHog's "Colision Dev" project with no PII attached.
